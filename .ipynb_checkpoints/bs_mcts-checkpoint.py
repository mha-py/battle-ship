...
#from isola_nnet import *
#from isola_gameclass import *
#from isola_helpers import *
#from isola_minimax import *
...
class PVTrivial:
    ...
    def predictStack(self):
        ...
        self.p_list = [ np.array([ 1/len(valid) for _ in valid ]) for valid in zip(self.valid_list) ]
        self.v_list = [ 0 for _ in self.state_list ]
        self.done = True
...
class PVnnet(PVTrivial):
    
    def __init__(self, net=None):
        import torch
        from bs_unet import bs_unet as Net
        if net is None:
            net = 'net.dat'
        if type(net) == str:
            self.nnet = Net()
    ...
    def predictStack(self):
        ...
        ###turn_list = [ s.turn for s in self.state_list ]
        ###self.state_list = [ s if s.turn>0 else GameIsola.exchangePlayers(s) for s in self.state_list ]
        
        ps, vs = self.nnet.predictStack(self.state_list, self.valid_list)
        ...
        # prepare output lists
        self.p_list = [ np.asarray([ parr[a] for a in valid ]) for parr, valid in zip(ps, self.valid_list) ]
        self.v_list = vs
        self.done = True
...
class MCTS:
    ...
    def findmove(self, state, tau=0, verbose=0):
        ...
        '''
        self.Nsa = GameIsola.numActions * [0]
        self.Qsa = GameIsola.numActions * [0]
        Q = [ self.Qa[s,a] for a in valid ]
        for k, a in enumerate(valid):
            self.Nsa[a] = Ns[k]
            self.Qsa[a] = Q[k]
        self.policy = np.asarray(self.Nsa) / np.sum(self.Nsa)'''
        ...
        if verbose>=3:
            ...
            #print(self.Nsalist)
            arr = np.zeros_like(state.board)
            arr2 = np.zeros((3,3))
            for k, a in enumerate(valid):
                l = a//9
                i, j = divmod(l, boardsize[1])
                arr[i,j] += self.Nsalist[k]
                l = a%9
                i, j = divmod(l, 3)
                arr2[i,j] += self.Nsalist[k]
            self.arr = arr    # hotness der Felder
            self.arr2 = arr2  # hotness der relativbewegungen
   ...
   # def search(self, state, s=None, nchosen=None, depth=0):
    ...
    def search_down(self, state):
        ...
        while True:
            ...
            # apply chosen move to get next game state
...
class BatchMCTS:
    def __init__(self, nparallel, nsearches, net=None, PV=None, params=None):
        '''Batch of MCTS move finders'''
        self.nsearches = nsearches
        self.nparallel = nparallel
        self.mcts = [ MCTS(nsearches=nsearches, PV='ignore')  for i in range(nparallel) ]
        self.PV = PV if PV else PVnnet(net)
